...
# Load LimitOrder ABI from Foundry artifact
try:
...
# Check balances
eth_balance = w3.eth.get_balance(owner_address)
weth_balance = weth.functions.balanceOf(owner_address).call()

print(f"[BALANCES] Native ETH: {w3.from_wei(eth_balance, 'ether')} ETH")
print(f"[BALANCES] WETH: {w3.from_wei(weth_balance, 'ether')} WETH")

# Step 1: Wrap ETH into WETH (0.00045 ETH = ~$0.96)
wrap_amount = w3.to_wei(0.00045, "ether")
if weth_balance < wrap_amount:
    print(f"\n[STEP 1] Wrapping {w3.from_wei(wrap_amount, 'ether')} ETH into WETH...")
    ...
    print(f"[TX] Wrap transaction sent: https://basescan.org/tx/{tx_hash.hex()}")
    w3.eth.wait_for_transaction_receipt(tx_hash)
    print("[SUCCESS] ETH successfully wrapped!")
else:
    print("\n[STEP 1] Already have enough WETH. Skipping wrap step.")

# Step 2: Approve our limit order contract to spend WETH
print(f"\n[STEP 2] Approving LimitOrder contract to spend WETH...")
nonce = w3.eth.get_transaction_count(owner_address)
tx = weth.functions.approve(CONTRACT_ADDRESS, wrap_amount).build_transaction({
    "from": owner_address,
    "gas": 45000,
    "maxFeePerGas": w3.eth.gas_price,
    "maxPriorityFeePerGas": w3.to_wei(0.001, "gwei"),
...
signed_tx = w3.eth.account.sign_transaction(tx, private_key)
tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
print(f"[TX] Approve transaction sent: https://basescan.org/tx/{tx_hash.hex()}")
w3.eth.wait_for_transaction_receipt(tx_hash)
print("[SUCCESS] LimitOrder contract approved to spend WETH!")
