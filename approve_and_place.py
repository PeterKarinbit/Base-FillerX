...
if weth_balance < wrap_amount:
    print(f"[ERROR] Wallet does not have enough WETH. WETH Balance: {weth_balance}")
    sys.exit(1)

# Step 1: Approve our limit order contract to spend WETH with a high gas limit
print(f"\n[STEP 1] Approving LimitOrder contract to spend WETH...")
nonce = w3.eth.get_transaction_count(owner_address)
tx = weth.functions.approve(CONTRACT_ADDRESS, wrap_amount).build_transaction({
    "from": owner_address,
    "gas": 100000, # Increased from 45000 to 100000 to prevent out of gas error
...
if receipt["status"] != 1:
    print("[ERROR] Approve transaction failed!")
    sys.exit(1)
print("[SUCCESS] LimitOrder contract approved successfully!")

# Step 2: Query current WETH/USDC spot price
...
receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
if receipt["status"] != 1:
    print("[ERROR] PlaceOrder transaction failed!")
    sys.exit(1)
